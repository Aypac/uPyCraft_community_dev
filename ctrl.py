...
class ctrlAction(QThread):
    ...
    def setDefaultProg(self, filename):
        ...
        if self.currentBoard == "microbit":
            ...
            # else:
            self.ctrltouartQueue.put(
                "ctrltouart:::myfile.write(\"exec(open(\'%s\').read(),globals())\")\r\n" % str(filename))
    ...
    def goProgram(self, filename):
        if str(filename).find("/") < 0:
            ...
            if (sys.platform == "linux" and str(filename).find(rootDirectoryPath) >= 0) or (
    ...
    def loadFile(self, filename):
        ...
        for afile in filenameB:
            if afile == "" or afile == ".":
                pass
            elif afile.find(".py") >= 0 or afile.find(".txt") >= 0 or afile.find(".json") >= 0 or afile.find(
                    ".ini") >= 0:
                connectdir += afile
                break
            else:
    ...
    def treeModel(self):
        ...
        if str(res).find("main.py") >= 0:
            ...
            self.ui.myDefaultProgram = self.reflushTreeMsg[
                                       str(self.reflushTreeMsg).find("('") + 2:str(self.reflushTreeMsg).find("')")]
        else:
