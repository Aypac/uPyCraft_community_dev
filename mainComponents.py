...
from PyQt5.QtWidgets import QTextEdit, QMenu, QAction, QMainWindow, QApplication, QTreeView, QAbstractItemView, \
    QTabWidget
    ...
    def slotRightClickMenu(self, point):
        ...
        if (self.ui.fileName == "." or self.ui.fileName == "/flash") and self.ui.myserial.ser.isOpen():
            ...
            pass
                ...
                (((sys.platform == "linux" and self.ui.fileName.find(rootDirectoryPath) >= 0) or (
                        sys.platform == "win32" and self.ui.fileName.find(":") >= 0) or (
                          sys.platform == "darwin" and self.ui.fileName.find(rootDirectoryPath) >= 0)) and
                 self.ui.fileName.split("/")[-1] == "uPy_lib"):
            pass
            ...
            self.rightClickMenu.addAction(self.closeFile)
                ...
                sys.platform == "win32" and self.ui.fileName.find(":") >= 0) or (
                      sys.platform == "darwin" and self.ui.fileName.find(rootDirectoryPath) >= 0)) and \
                self.ui.fileName.split("/")[-1] == "workSpace":
            self.rightClickMenu.addAction(self.openFile)
        elif ((sys.platform == "linux" and self.ui.fileName.find(rootDirectoryPath) >= 0) or (
                sys.platform == "win32" and self.ui.fileName.find(":") >= 0) or (
                      sys.platform == "darwin" and self.ui.fileName.find(
        ...
        if event.mimeData().hasUrls:
            ...
            if self.dragFrom == "External":
                ...
                if dirList[1] == "device" and str(dirList[-1]).find(".") < 0:
                    ...
                    shutil.copyfile(dropDownFileName,
                                    self.ui.workspacePath[0:-10] + self.dropDir + "/" + dropDownFileName.split("/")[-1])
                ...
                if str(oldDragFileName).find(".") < 0:
                    self.ui.terminal.append("Now version only support drag file, no support for dir.")
                    return
                ...
                if dirListDrag[1] == "device" and str(dirListDrop[-1]).find(".") < 0:
                    if oldDragFileName[0:str(oldDragFileName).find(dirListDrag[-1]) - 1] == newDropFileName:
                        self.ui.terminal.append("The same dir, won't move.")
                        return
...
class myTabWidget(QTabWidget):
    ...
    def createNewTab(self, filename, msg, lexer):
        ...
        #    msg=msg.replace("\n","\r")
        # else:
        #    print("create NewTab has other endswith.")
        msg = msg.replace("\r\n", "\r")
        msg = msg.replace("\n", "\r")
        ...
        elif (sys.platform == "linux" and str(filename).find(rootDirectoryPath) >= 0) or (
                sys.platform == "win32" and str(filename).find(":") > 0) or (
                sys.platform == "darwin" and str(filename).find(rootDirectoryPath) >= 0):
            self.tabBar().setTabTextColor(self.count() - 1, QColor(Qt.red))
            self.setTabIcon(self.count() - 1, QIcon(':/pc.png'))
        else:
        ...
        if linetext.find("\r") >= 0 or linetext.find('\n') >= 0:
            ...
            if linetext[-1] == ".":
                ...
                for i in linetext[0:-1]:
                    if ord(i) <= 0x2F or (ord(i) >= 0x3A and ord(i) <= 0x40) or (ord(i) >= 0x5B and ord(i) < 0x5F) or (
                            ord(i) > 0x5F and ord(i) <= 0x60) or \
    ...
    def userListActivated(self, ida, text):
        ...
        if linetext == "":
            ...
            self.currentWidget().setCursorPosition(self.line,
                                                   self.index + len(text[str(text).find(linetext) + len(linetext):]))
